...
namespace DAL
{
    public class WorkInfo
    {
        ...
        public IEnumerable<Model.WorkInfo> GetAll()
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
        ...
        public int GetCount(int[] activityIds)
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
                string sql = "select count(1) from workinfo where activityId in @activityIde";
        ...
        public int GetFindCount(string workName)
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
                string sql = "select count(1) from workinfo where workverify='审核未通过' and workName like concat('%',@workName,'%')";
